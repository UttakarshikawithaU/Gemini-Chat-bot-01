...
def user_input(user_question, raw_text, ground_truth_answer, domain):
    ...
    # print("These are chunky boisss !! :",text_chunks)
    ...
    print("who let these docs out? :",docs)
...
def extract_visible_text(web_link):
                    ...
                    try:
                        ...
                        # Extract text, handling common elements and filtering out non-essential content
                        visible_text = ' '.join(
                            ...
                            and not line.isspace()  # Remove empty lines
                        )
...
def main():
    ...
    with st.sidebar:
        ...
        web_link = st.text_input("Enter the URL of the web page")
   
        ground_truth_answer = st.text_input("Enter Ground Truth Answer for Evaluation")

        if st.button("Submit & Process"):
            with st.spinner("Processing..."):
                if user_question and ground_truth_answer and domain:
                    ...
                    #     else:
                    #         st.error("Failed to fetch context from the provided URL. Please check URL & try again.")
                    #         return 

                    # Example usage (assuming you have streamlit imported as st)
                    if web_link:
                        visible_text += extract_visible_text(web_link)
                        if visible_text:
                            #print("\n\nExtracted visible text:\n", visible_text)
                            print("\n\nExtracting visible text:  done\n\n")
                        else:
                            st.error("Failed to extract text. Please check the URL and try again.")

                    
                    output = user_input(processed_question['concatenated_string'], visible_text, ground_truth_answer, domain)
                    st.write("Response:", output["response"])
                    
                    # Save raw_text as PDF
                    with open("C:/Users/VIBGYOR/Desktop/LLM-Mini-Resources/pdfgenerated.pdf", "wb") as f:
                        f.write(raw_text.encode('utf-8'))
                    st.success("PDF saved successfully at: C:/Users/VIBGYOR/Desktop/LLM-Mini-Resources/pdfgenerated.pdf")
