...
# #     if tone is not None:
# #         style_and_tone_variable = " Keep your tone of answer {}".format(tone)
# #     else:
# #         style_and_tone_variable = "Use 10th-grade language and explain things in a simple way that anyone can understand"
...
# #                     st.markdown(processed_question["concatenated_string"])
...
#                 if messages:
#                     messages.chat_message("user").write(processed_question["concatenated_string"])
#                     output = user_input(processed_question['concatenated_string'], pdf_docs, ground_truth_answer, domain)
#                     messages.chat_message("assistant").write(output["response"])
#             else:
#                 st.warning("Please provide input for all fields")
...
def user_input(user_question, pdf_docs, ground_truth_answer, domain, history):
    
    llm = ChatGoogleGenerativeAI(model="gemini-pro", temperature=0.1)
    memory = ConversationSummaryMemory(llm=llm, return_messages=True, initial_context=history)
    summary_memory_chain = ConversationChain(
        llm=llm,
        memory=memory,
        verbose=True
...
def main():
    ...
    if messages:
        for msg in history:
            if msg["sender"] == "User":
                messages.write(f"User: {msg['message']}")
            else:
                messages.write(f"Assistant: {msg['message']}")
    ...
    if st.button("Submit & Process"):
        with st.spinner("Processing..."):
            if user_question and pdf_docs and ground_truth_answer and domain:
                processed_question = preprocess_question(user_question, domain, tone)
                if messages:
                    messages.write(f"user: {processed_question['concatenated_string']}")
                    output, history_print = user_input(processed_question['concatenated_string'], pdf_docs, ground_truth_answer, domain, history)
                    messages.write(f"assistant: {output['response']}")
                    history.append({"sender": "user", "message": processed_question['concatenated_string']})
                    history.append({"sender": "assistant", "message": output['response']})
            else:
                st.warning("Please provide input for all fields")
