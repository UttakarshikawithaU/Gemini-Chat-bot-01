...
# from langchain.prompts import ChainGPTTemplate
...
priming_variable = "You are an expert that provides in-detail answers with reference to the questions in the 'passed' domain."
...
# Expanding question for better user exp
...
def user_input(user_question, pdf_docs, ground_truth_answer, use_gemini_knowledge, domain):
    ...
    relevance_score = load_qa_chain(ChatGoogleGenerativeAI(model="gemini-pro", temperature=0.7),chain_type="stuff",
    ...
    # # Convert filtered_docs to a list
    # filtered_docs = filtered_docs.split('\n')  # Assuming the string is newline-separated
    
    # # Check the data type of filtered_docs
    # filtered_docs_type = type(filtered_docs)
    # print("Data type of filtered_docs:", filtered_docs_type)
    st.write("Filtered Doc: ", filtered_docs)
    
    # # Convert filtered_docs to a list of documents
    # documents = filtered_docs  # Assuming each string represents a document

    # # Check the data type of documents
    # documents_type = type(documents)  
    # print("Data type of documents:", documents_type)
    # st.write("Filtered Doc: ", documents)  
    ...
    response = chain({"input_documents": filtered_documents, "question": user_question}, return_only_outputs=True)
        ...
        if st.button("Submit & Process"):
            with st.spinner("Processing..."):
                if user_question and pdf_docs and ground_truth_answer and domain:
                    ...
                    user_input(processed_question['concatenated_string'], pdf_docs, ground_truth_answer, use_gemini_knowledge, domain)
                else:
